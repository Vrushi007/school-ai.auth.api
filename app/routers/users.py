...
async def create_user(
    ...
    auto_generate_password: bool = True,  # Default to auto-generate
    db: Session = Depends(get_db),
    current_user: User = Depends(get_school_admin_or_higher)
):
    """
    Create a new user. If auto_generate_password is True, password is generated by system.
    User is activated and sent email if created by admin.
    """
    db_user = user_service.create_user(db, user_create, auto_generate_password=auto_generate_password, created_by_admin=True)
    response = UserResponse.model_validate(db_user)
    if hasattr(db_user, "generated_password") and db_user.generated_password:
        response.generated_password = db_user.generated_password
    return response
